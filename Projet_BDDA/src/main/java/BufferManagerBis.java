        ...
        if (frameDispo.isEmpty()) { // Si plus de frame libre
            ...
            if(!framePC0.isEmpty()){ // Vérification de la présence d'au moins un buffer avec un pin count =O
                ...
                if ( bufferMap.get(indiceBuffer ).get(1).equals(true) ){ // Vérification du buffer pour voir s'il a été modifer (dirty à 1)
                    ...
                    diskManager.WritePage((PageId) bufferMap.get(indiceBuffer).get(0), bufferPool[indiceBuffer].array()); // on inscrit les changements que le précédent buffer a fait sur le disque
                ...
                diskManager.ReadPage(pageId, bufferPool[indiceBuffer].array()); // Strockage de la page dans le buffer
