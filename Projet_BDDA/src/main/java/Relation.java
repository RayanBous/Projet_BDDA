...
public class Relation {
    ...
    public PageId getFreeDataPageId(int sizeRecord) {
        ...
        try {
            ...
            System.out.println("GetFreeDataPageID : BUFFERHEADERPAGE : " + bufferHeaderPage);
        } catch (IOException e) {
            e.printStackTrace();
        }
        ...
        if (bufferHeaderPage == null) {
            System.out.println("GetFreeDataPageId : ON RENTRE DANS LE CAS OU LE BUFFERHEADERPAGE EST NULL");
            bufferHeaderPage = new MyBuffer(this.headerPageId, this.diskManager.getDBConfig().getPagesize(), this.bufferManager.getTimeCount());
            System.out.println("GetFreeDataPageId : BUFFERHEADERPAGE APRES NULL : " + bufferHeaderPage);
        }
        ...
        for (int i = 0; i < nbDataPages; i += 1) {
            ...
            if (availableSpace >= sizeRecord + 8) {
                ...
                System.out.println("GetFreeDataPageID : FREEDATAPAGE : " + freeDataPage);
                break;
            }
        }
        ...
        for (ColInfo c : this.colonnes) {
            switch (c.getType()) {
                case "INT":
                    ...
                    System.out.println("RECORD SIZE INT : " + recordSize);
                    break;
                case "REAL":
                    recordSize += Integer.BYTES;
                    System.out.println("RECORD SIZE REAL : " + recordSize);
                    break;
                case "CHAR":
                    recordSize += (Character.BYTES * c.getTaille());
                    System.out.println("RECORD SIZE CHAR : " + recordSize);
                    break;
                case "VARCHAR":
                    System.out.println("RECORD SIZE VARCHAR : " + Arrays.toString(record.getAttributs()));
    ...
    public List<PageId> getDataPages() {
        ...
        System.out.println("DBCONFIG : " + this.diskManager.getDBConfig());
        long pageSize = this.diskManager.getDBConfig().getPagesize();
        MyBuffer bufferHeaderPage = null;
        try {
            bufferHeaderPage = this.bufferManager.getPage(this.headerPageId);
    ...
    public RecordId InsertRecord(Record record) {
        if (record == null) {
            System.err.println("ERREUR : Le record fourni est nul. Insertion annulée.");
            return null;
        }

        int recordSize = this.getRecordSize(record);
        if (recordSize <= 0) {
            System.err.println("ERREUR : La taille du record est invalide. Insertion annulée.");
            return null;
        }

        PageId pageId = this.getFreeDataPageId(recordSize);
        System.out.println("RELATION INSERT RECORD : Page libre initiale : " + pageId);

        if (pageId == null) {
            System.out.println("Aucune page libre trouvée. Création d'une nouvelle page de données.");
            addDataPage();
            pageId = this.getFreeDataPageId(recordSize);
            if (pageId == null) {
                System.err.println("ERREUR : Échec de la création d'une nouvelle page pour insérer le record.");
                return null;
            }
            System.out.println("Nouvelle page créée et sélectionnée : " + pageId);
        }

        System.out.println("RELATION INSERT RECORD : Record à insérer : " + Arrays.toString(record.getAttributs()));
        RecordId recordId = this.writeRecordToDataPage(record, pageId);
        if (recordId == null) {
            System.err.println("ERREUR : Échec de l'insertion du record dans la page " + pageId);
            return null;
        }

        System.out.println("Record inséré avec succès. RecordId : " + recordId);
        return recordId;
    }
    ...
    public List<Record> GetAllRecords() {
        ...
        System.out.println("LIST<PAGEID> pages valeurs : " + pages);
        System.out.println("RELATION : Get All Records : liste des pages disponible : " + getDataPages());
        for (PageId pageId : pages) {
            List<Record> record = this.getRecordsInDataPage(pageId);
            System.out.println("GET ALL RECORDS : LIST<RECORD> record : " + record);
            allRecords.addAll(record);
            System.out.println("GET ALL RECORDS : LIST<RECORD> record après addAll : " + record);
        }
        System.out.println("RELATION : GET ALL RECORDS : TEST : " + allRecords);
        return allRecords;
    }
